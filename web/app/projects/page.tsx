...
export default async function ProjectsPage({ searchParams }: { searchParams?: { q?: string, type?: string } }){
  ...
  const filtered = items.filter(p=> byTag(p) && (
    q ? ((p.title||'').toLowerCase().includes(q) || (p.summary||'').toLowerCase().includes(q) || (Array.isArray(p.tags)?p.tags.join(',').toLowerCase().includes(q):false)) : true
  ))
  return (
    <FeedLayout trending={items} news={data.companies} title="作品">
      <FacetFilter facets={[{ key:'tags', label:'标签', options: tagSet }]} />
      <div className="divide-y divide-[color:var(--border)]">
        {filtered.map(p=> (
          <FeedRow key={p.id} title={p.title} subtitle={p.summary} tags={p.tags} />
        ))}
      </div>
    </FeedLayout>
  )
